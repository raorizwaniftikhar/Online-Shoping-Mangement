...
class WebPages{
	...
	function add_page($fields_arr){
		...
		$title=$fields_arr['title'];
		$description=$fields_arr['description'];
		$text=$fields_arr['text'];
		$section=$fields_arr['section'];
		$category_id=$fields_arr['category_id'];
		$locked='no';
		return $this->db->query("insert into pages (id,name,title,description,text,section,locked) values ('','$name','$title','$description','$text','$section','$locked')");
	}
...
?>
		...
		function show_sub_cat(cat){
			if(cat=='footer'){
				document.getElementById('menu_sub').style.display='';
			}else{
...
?>		
		<form name="frm1" method="post" action="" onSubmit="return validate()" enctype="multipart/form-data" >
            <table>
                ...
                <tr>
                	<td>Description: </td>
                    <td><textarea name="description" id="content1" rows="5" cols="50"><?php echo $description; ?></textarea></td>
                </tr>
                <tr>
                	<td>Text: </td>
                ...
                <tr>
                	<td>Section: </td>
                    <td>
                    	<select name="section" onchange="show_sub_cat(this.value)">
                ...
                <tr id="menu_sub" style="display:none" >
                	<td>Menu Section: </td>
                    <td>
                    	<select name="category_id">
                        <option value="0">Select</option>
                        <?php $cat_result=$this->db->query("select * from menu_categories");
                    ...
                    <script language="javascript">
						document.frm1.category_id.value='<?php echo $category_id; ?>';
